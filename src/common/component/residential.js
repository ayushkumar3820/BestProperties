...
export default function Residential() {
  const { our_services } = useParams(); // Destructure for cleaner code
  const navigate = useNavigate(); // Use consistent naming
  const [newData, setNewData] = useState([]); // Initialize as empty array
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${liveUrl}api/Services/servicesInnerPages`, {
        ...
        body: JSON.stringify({
          innerpagetitle: our_services || "", // Fallback to empty string
        }),
      });
      ...
      setNewData([]); // Reset to empty array on error
    } finally {
      setLoader(false);
    }
  };
  ...
  useEffect(() => {
    handleSubmit();
  }, [our_services]); // Add our_services as dependency to refetch on param change

  const formatBudget = (value) => {
    if (!value || isNaN(value)) return "N/A"; // Handle invalid values
            ...
            {newData.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-5 justify-items-center">
                {newData.map((panel, index) => (
                  <div
                    key={panel.id || index} // Use unique key
                    onClick={() => {
                  ...
                  >
                    <div className="relative">
                      <div className="flex items-center bg-green-900 text-white px-2 py-2 absolute bottom-0 left-0 font-bold text-sm lg:text-base">
                        ...
                        {formatBudget(panel.budget)} / {panel.sqft || "N/A"}{" "}
                        {panel.measureUnit || ""}
                      </div>
